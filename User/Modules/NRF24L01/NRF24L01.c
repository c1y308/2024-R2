...
#include "spi.h"
 ...
 int16_t vx_control = 0;
 int16_t vy_control = 0;
 int16_t vz_control = 0;
 int ball_control = 0;
//NRF24L01 驱动函数
...
const unsigned char INIT_ADDR0[5]= {0x04,0x3A,0x01,0x01,0x02};
const unsigned char INIT_ADDR1[5]= {0x04,0x3A,0x01,0x01,0x02};
...
unsigned char nRF24_SPI_Send_Byte(unsigned char txdata)
{
	...
	HAL_SPI_TransmitReceive(&hspi3, &txdata, &rxdata, 1, 0x10);
	...
	unsigned char datalen)
{
	...
	HAL_SPI_Receive(&hspi3, pBuf, datalen, 0x10);
	...
	unsigned char datalen)
{
	...
	HAL_SPI_Transmit(&hspi3, pBuf, datalen, 0x10);
	Set_NRF24L01_CSN;                                    //关闭SPI传输
	return status;                                       //返回读到的状态值
}
//启动NRF24L01发送一次数据
...
void nRF24L01_Rx_AckPayload(uint8_t *tx_buf, uint8_t ch)
{
	...
	NRF24L01_Write_Buf(SPI_WRITE_REG | WR_ACK_PAYLOAD | ch, tx_buf, 8);//填入数据
}


//uint8_t rx_buf[8] = {0};
//uint8_t tx_back_buf[8] = "sendBack";
//int16_t vx = 0;
//int16_t vy = 0;
//int16_t vz = 0;
//int ball =0;
//void rx_remotcontrol(){	
//if(NRF24L01_RxPacket_ack_data(rx_buf) == 0){
//	nRF24L01_Rx_AckPayload(tx_back_buf,1);
//	if(rx_buf[0] == 0x0a && rx_buf[7] == 0xff){
//		if(rx_buf[1] == 0x00){
//			vx = (int16_t)((rx_buf[2] <<8 | rx_buf[3]) - 1000) * 5;
//			vy = (int16_t)((rx_buf[4] <<8 | rx_buf[5]) - 1000) * 5;
//			vz = (int16_t)(rx_buf[6]  - 100) * 5;
//			
//			
//		}
